<?php
  ...
  $sql_status_request = mysqli_query($conn, "SELECT * from company_recruitment_request_form WHERE id=$request_id");
  $status_company_recruitment_request_form = mysqli_fetch_assoc($sql_status_request);
?>
...
<div class="w3-col s6">
  <div class="w3-row">
    ...
    <div class="w3-row">
      ...
      <?php
        if($status_assigned_table["status"] == "0" || $status_assigned_table["status"] == "1" ||$status_company_recruitment_request_form["statuss"] == "4000" ){?>
          <button type="button" disabled>Register</button>
      <?php } else { ?>
        <button><a href="../server/student/request_registration_company.php?id=<?php echo $request_id;?>&student_id=<?php echo $_SESSION['student_id']?>&date=<?php echo date('Y/m/d');?>">Register</a></button>
      <?php } ?>
    </div>
  </div>
</div>
