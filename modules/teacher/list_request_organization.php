...
<?php while($temp = mysqli_fetch_assoc($sql)) { ?>
  <div class="w3-col w3-border content w3-round">
    ...
    <div class="w3-row w3-center">
      <button><a href="teacher.php?status=request_organization&request_id=<?php echo $temp['id'];?>&organization_id=<?php echo $temp['organization_id'];?>" class="w3-button w3-white w3-border w3-border-grey w3-round-large">Xem chi tiết</a></button>
      <button><a href="teacher.php?status=list_assigned&request_id=<?php echo $temp['id']?>" class="w3-button w3-white w3-border w3-border-grey w3-round-large">Xem bảng phân công</a></button>
    </div>
  </div>
<?php } ?>
